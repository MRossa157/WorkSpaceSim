...
# Константы игры
SIMULATION_SPEED = 2  # минут за кадр при нормальной скорости (было 5)
ZOOM = 1.0
OFFSET_X = 0
OFFSET_Y = 0
...
class WorkSpaceSimApp:
    ...
    def __init__(self, seed=None):
        ...
        self.simulation.initialize(worker_count=DEFAULT_WORKER_COUNT)

        # Состояние интерфейса
        self.paused = False
        self.speed_multiplier = BASE_SIMULATION_SPEED * SPEED_MULTIPLIER_1  # Начальная скорость
        self.current_speed_level = SPEED_MULTIPLIER_1  # Текущий уровень скорости (1, 2 или 3)
        ...
        for event in pygame.event.get():
            ...
            # События клавиатуры
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    self.current_speed_level = SPEED_MULTIPLIER_1
                    self.speed_multiplier = BASE_SIMULATION_SPEED * SPEED_MULTIPLIER_1
                elif event.key == pygame.K_2:
                    self.current_speed_level = SPEED_MULTIPLIER_2
                    self.speed_multiplier = BASE_SIMULATION_SPEED * SPEED_MULTIPLIER_2
                elif event.key == pygame.K_3:
                    self.current_speed_level = SPEED_MULTIPLIER_3
                    self.speed_multiplier = BASE_SIMULATION_SPEED * SPEED_MULTIPLIER_3
            ...
            # События мыши
                ...
                if event.button == 1:  # Левый клик
                    ...
                    self.zoom *= ZOOM_STEP
                    self.zoom = min(MAX_ZOOM, self.zoom)
    ...
    def _handle_click(self, pos):
        ...
        if x < SCREEN_WIDTH - panel_width:
            ...
            for worker in self.simulation.workers.values():
                ...
                radius = 10 * self.zoom
                if ((worker_screen_x - x) ** 2 + (worker_screen_y - y) ** 2) <= radius ** 2:
        ...
        if self.show_info_panel:
            seed_box_rect = pygame.Rect(
                SCREEN_WIDTH - self.info_panel_width + 10,
                140,
    ...
    def _draw_office(self):
        ...
        self.office_display_surface.fill(WHITE)

        # Рисуем комнаты
        for room in self.simulation.rooms:
            color = ROOM_COLORS.get(room.room_type, GRAY)
            ...
            room_label = self.font.render(room.room_type.value, True, BLACK)
            ...
            for i, event in enumerate(room.events):
                ...
                event_label = self.font.render(event_text, True, RED)
                self.office_display_surface.blit(event_label, (x + 5, y + 25 + i * 15))
    ...
    def _draw_workers(self):
        ...
        for worker_id, worker in self.simulation.workers.items():
            ...
            radius = SELECTED_WORKER_CIRCLE_RADIUS if worker == self.selected_worker else WORKER_CIRCLE_RADIUS
            radius *= self.zoom
            ...
            if worker.current_task:
                progress = worker.current_task.progress / worker.current_task.duration
                indicator_width = PROGRESS_BAR_WIDTH * self.zoom
                pygame.draw.rect(
                    self.office_display_surface,
                    BLACK,
                ...
                pygame.draw.rect(
                    self.office_display_surface,
                    GREEN,
                    (x - indicator_width/2, y - 15, indicator_width * progress, 5)
                )

    def _draw_info_panel(self):
        """Отрисовка информационной панели."""
        panel_x = SCREEN_WIDTH - self.info_panel_width
        panel_rect = pygame.Rect(panel_x, 0, self.info_panel_width, SCREEN_HEIGHT)
        ...
        # Рисуем заголовок
        title = self.title_font.render("WorkSpaceSim", True, BLACK)
        self.screen.blit(title, (panel_x + 10, 10))

        # Рисуем время симуляции
        time_text = self.simulation.get_current_time_str()
        time_label = self.info_font.render(time_text, True, BLACK)
        self.screen.blit(time_label, (panel_x + 10, 80))

        # Рисуем ввод сида
        seed_label = self.info_font.render("Сид:", True, BLACK)
        self.screen.blit(seed_label, (panel_x + 10, 110))

        seed_box_color = LIGHT_BLUE if self.seed_input_active else WHITE
        pygame.draw.rect(
            self.screen,
            seed_box_color,
            (panel_x + 10, 140, self.info_panel_width - 20, 30)
        )
        seed_value = self.info_font.render(self.seed_input_text, True, BLACK)
        self.screen.blit(seed_value, (panel_x + 15, 145))

        # Рисуем информацию о скорости
        if self.current_speed_level == SPEED_MULTIPLIER_1:
            speed_label = SPEED_SLOW_LABEL
        elif self.current_speed_level == SPEED_MULTIPLIER_2:
            speed_label = SPEED_NORMAL_LABEL
        else:
            speed_label = SPEED_FAST_LABEL

        speed_text = f"Скорость: {speed_label} (×{self.current_speed_level})"
        if self.paused:
            speed_text += " (Пауза)"
        speed_info = self.info_font.render(speed_text, True, BLACK)
        ...
        for i, control in enumerate(controls):
            control_label = self.font.render(control, True, BLACK)
    ...
    def _draw_worker_info(self, panel_x):
        ...
        worker_title = self.info_font.render(
            f"Работник: {self.selected_worker.name}",
            True,
            BLACK
        )
        ...
        for i, info in enumerate(worker_info):
            info_label = self.font.render(info, True, BLACK)
    ...
    def _draw_task_info(self, panel_x, task_y):
        ...
        task_title = self.info_font.render("Текущее задание:", True, BLACK)
    ...
    def _draw_task_panel(self):
        ...
        panel_width = TASK_PANEL_WIDTH
        panel_height = TASK_PANEL_HEIGHT
        ...
        # Рисуем заголовок
        title = self.title_font.render("Доступные задания", True, BLACK)
        self.screen.blit(title, (panel_x + 10, panel_y + 10))

        # Рисуем список заданий
        for i, task in enumerate(self.simulation.available_tasks[:MAX_TASKS_DISPLAYED]):
            y_pos = panel_y + 40 + i * 20
            task_text = f"{task.name} - {task.duration}мин"
            task_label = self.font.render(task_text, True, BLACK)
            self.screen.blit(task_label, (panel_x + 10, y_pos))

        # Рисуем инструкции по закрытию
        close_text = "Нажмите T для закрытия"
        close_label = self.font.render(close_text, True, BLACK)
        self.screen.blit(
            close_label,
